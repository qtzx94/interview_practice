...
function Promise(executor) {
    ...
    function resolve(value) {
        if(that.status === PENDING) {
            ...
            that.value = value;
            that.onFulfilled.forEach(fn => fn()); // PromiseA+ 2.2.6.1
        }
    }
    ...
    function reject(reason) {
        if(that.status === PENDING) {
            ...
            that.reason = reason;
            that.onRejected.forEach(fn => fn()); // PromiseA+ 2.2.6.2
        }
    }
...
Promise.prototype.then = function(onFulfilled, onRejected) {
    // PromiseA+ 2.21 / PromiseA+ 2.2.5 / PromiseA+ 2.2.7.3 / PromiseA+ 2.2.7.4
    const that = this;
